...
def add_faculty(username, email, password_hash, first_name, last_name, dept_id, designation=None):
    """Adds a new faculty by inserting into Users then Faculty.

    designation is optional; existing callers that omit it are unaffected.
    When provided (from the Admin 'Add Faculty' form) it is stored in the
    designation column (requires sql/add_faculty_designation.sql migration).
    """
    user_query = '''
            ...
            with conn.cursor() as cur:
                ...
                if designation is not None:
                    faculty_query = '''
                        ...
                        VALUES (%s, %s, %s, %s, %s)
                    '''
                    cur.execute(faculty_query,
                                (user_id, first_name, last_name, dept_id, designation))
                else:
                    faculty_query = '''
